...
let props = defineProps({
    role: Object,
    routeName: { type: String, default: 'roles.store' },
...
let submit = () => {
    ...
    if (props.isEdit && props.role) {
        form.put(route(config.admin_route_name + actionRoute, props.role.id), {
            onSuccess: () => {
                form.reset();
            },
        });
    ...
    <form @submit.prevent="submit" class="w-full max-w-full bg-gray-800 rounded shadow p-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
            <div class="space-y-2">
                ...
                <Input id="name" type="text" v-model="form.name" required placeholder="Enter Role Name" class="w-full shadow-xl" />
